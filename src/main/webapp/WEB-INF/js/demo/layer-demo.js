...
//友情赞助
    ...
    juan.zdlist = function (times) {
        var ii = $.layer({
            ...
            success: function (layerE) {
                var closeZD = layerE.find('.closeZD'), zzmain = layerE.find('#zzmain'), zdcout = layerE.find('#zdcout'),
                    couts = 0;
                setTimeout(function () {
...
//初始即执行
    conf.start = (function () {
        ...
        if (conf.ie6) {
            setTimeout(function () {
                layer.msg('如果您是用ietest的ie6模式，发现弹出背景一片黑色时，请不用惊慌，这并非layer未作兼容，而是你当前版本的ietest所模拟的ie6环境未对滤镜做支持，标准ie6将不会有此问题，所以请您不要担心。', 15, {
                    shade: false,
                    type: 9
                });
            }, 1000);
