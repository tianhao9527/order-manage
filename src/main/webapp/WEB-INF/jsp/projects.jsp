            ...
            <div class="col-sm-4">
                ...
                <ol class="breadcrumb">
                    <li><a href="/index/showIndex">主页</a></li>
                        <c:if test="${category.categoryName ==null || category.categoryName=='' }">
                    ...
                    <div class="ibox">
                        <div class="ibox-title">
                            ...
                            <div class="ibox-tools">
                                <a href="${pageContext.request.contextPath}/order/export" class="btn btn-primary btn-xs">导出账单</a>
                            </div>
                            <shiro:hasRole name="user">
                                <div class="ibox-tools">
                                    <%--<input type="file" id="upfile" name="upfile" style="display:none"></input>--%>
                                    <a id="importBtn" href="javascript:" class="btn btn-primary btn-xs">导入账单</a>
                                </div>
                                <div class="ibox-tools">
                                    <a href="/order/downmodel?fileName=model.xls" class="btn btn-primary btn-xs">下载Exel模板</a>
                                </div>
                                <div class="ibox-tools">
                                    <a href="/order/add/${category.id}" class="btn btn-primary btn-xs">创建新账单</a>
                                </div>
                            </shiro:hasRole>
                        </div>
                        <div class="ibox-content">
                            <div class="row m-b-sm m-t-sm">
                                ...
                                <form id="searchForm" method="post" action="/order/search">
                                    <div class="col-md-11">
                                        <div class="input-group">
                                            <input type="text" name="orderName" placeholder="请输入项目名称"
                            ...
                            <div class="project-list">
                                ...
                                <table class="table table-hover">
                                    ...
                                    <c:forEach items="${orders}" var="order">
                                        <tr>
                                            <td class="project-title">${order.orderName }
                                                <br/>
                                                <small>创建于 ${order.createtime }</small>
                                            </td>
                                            <td class="project-title">
                                                <br/>
                                                <small>最近一次修改 ${order.updatetime }</small>
                                            </td>
                                            <td class="project-actions">
                                                <a href="${pageContext.request.contextPath}/order/order_detail/${order.orderId }" class="btn btn-white btn-sm">
                                                    <i class="fa"> </i> 查看
                                                </a>
                                                <shiro:hasRole name="user">
                                                    <a href="${pageContext.request.contextPath}/order/order_detail/${order.orderId }" class="btn btn-white btn-sm"><i
                                                            class="fa"></i> 编辑 </a>
                                                    <i class="fa">
                                                    <form>
                                                        <input type="hidden" name="orderId" value="${order.orderId }">
                                                        <input type="submit" myAttr="none" class="btn btn-white btn-sm" value="删除"></input>
                                                    </form>
                                                    </i>
                                                </shiro:hasRole>
                                            </td>
...
<!-- 文件上传 -->
<script src="${pagecontext.request.getcontextpath}/js/myjs/ocupload/jquery.ocupload-1.1.2.js";></script>
...
<script type="text/javascript">
    $(function(){
        ...
        $("#importBtn").upload({
            // 默认name为file
            action : '<%=request.getContextPath()%>/order/import',
            onSelect :function(){
                // 选中文件后，关闭自动提交
...
<script type="text/javascript">
 $("[myAttr]").click(function () {
     ...
     $.post("/order/delete",{orderId:orderId},function (data) {
         if(data.status==200){
             alert("删除成功");
         }else{
             alert("删除失败");
